...
describe("BookId", () => {
  ...
  test("不正な文字数の場合にエラーを投げる", () => {
    // 境界値のテスト
    expect(() => new BookId("1".repeat(101))).toThrow("isbn is invalid");
    expect(() => new BookId("1".repeat(9))).toThrow("isbn is invalid");
  });

  test("不正なフォーマットの場合にエラーを投げる", () => {
    expect(() => new BookId("9994167158057")).toThrow("isbn is invalid");
  });
});
